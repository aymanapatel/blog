...
2. Document-oriented: Mongo
    
3. Column Family: HBase
    
4. Graph: Neo4j
    

![](../images/NoSQL-Database.png align="left")

Funnily, Postgres has a solution for all these NoSQL horsemen!

## Key Value store

KV are basically a simple data structure where there is a key by which you query your values that could be of many different data structures such as string, map, sets, hashes etc. KV stores such as Redis and Memcachced became popular in the 2010s for caching use-cases.

Postgres has an extension called `hstore`, which helps in creating a data type `hstore` which can be used as a K-V column
...
Column family or column-oriented databases are used in Big Data applications.

Postgres has a couple of solutions for this

1. [Citus](https://www.citusdata.com/): A HTAP database that has capability of being. column-oriented database, Citus wll be discussed below
    
2. [Hydra](https://www.hydra.so/): A fork of Citus that is purely focused on being a columnar analytics database.
...
As Graph databases such as Neo4j has not become a mainstream. But at least, there is a Postgres extension(more on it later!) which is from the Apache foundation called **AGE**.

It is an extension that hooks into Postgres and also supports the typical Graph Database querying language called **Cypher**.(SQL can also be used for querying)

Finally, it also had a visualisation tool to show the data points and connections in the Graph Database.

## Document Oriented

In the early 2010s, a new kid in the block had come from the ashes and that was Mongodb. It was meant to be "scalable" and "flexible" as it worked on collection. If you want to simplify it, it could be thought of a flexible JSON that is stored in database. Initially, Mongo sacrificed speed for data durability and as it became more prevelant in big applications, the lack of ACID compliance became a big blow to Mongo. Mongo therefore started added Transactions and changed their storage from mmap to Wiredtiger.

Postgres (and other databases) looked into supporting JSON as a data type. Postgres has both JSON and JSONB(B stands for binary); along with neat helper functions for querying keys in the database. I wrote more on this here
...
All the 4 alternatives shows how Postgres always has an answer to the new database paradigms. It might sound like Postgres is becoming a "jack of all trades, master of none"; but one can argue that having wrangled data systems for each type of database can be an overhead in terms of overall performance and cost of maintainence.
...
It is touted as an "OSS alternative to Firebase", which is quite an undersell. It does a lot of what Firebase does; but its key advantages are:
...
![](../images/supabase_sql.webp align="left")

2. [Authentication](https://supabase.com/docs/guides/auth) Supabase provides authentication and authorization with help of Supabase Client SDKs for UI frameworks such as React, Kotlin, Next.js, Flutter etc. Supabase uses JWTs for authentication. As it is based on Postgres, Supabase also has **Row level security** for authorized access at the row-level. Supabase provides social logins such as for Google, Apple etc.
    
3. [Storage](https://supabase.com/docs/guides/storage) S3 compatible object storage which you can use to store images, CSVs and other files
    
4. [Edge Functions](https://supabase.com/docs/guides/functions) Based on Deno, it allows to write Globally distributed Functions to execute small but fast one-off functions such as Payment etc
    
5. [Real-time](https://supabase.com/docs/guides/realtime) communication between clients
...
4. [Vector](https://supabase.com/docs/guides/ai) Create vectors from data and then proceed to create embeddings for AI apps. It uses `pg_vector` to create a Vector store.
...
Neon is a serverless Postgress which is highly distributed, serverless database that can scale down to zero. It is inspired by Amazon Aurora which is one of mainstream databases solutions that was architected to **separate compute and storage**.

Neon replaces the Postgres storage layer by its own storage engine. Neon storage engine consists of:

![](../images/neon_architecture.jpeg align="left")
...
![](../images/YugabyteDB_Architecture.png align="left")
...
* Storage: DocDB is the storage engine that is used here. It is based out of [Rocks DB](http://rocksdb.org/) (a [Log-Structured Merge Tree](https://docs.yugabyte.com/preview/architecture/docdb/lsm-sst) KV store from Facebook). The tables are splitted into tablets for partitioning purposes.
...
Database are not just a sequence of rows and columns with varying types. There are some data that are high-volume datasets that are temporal in nature(time-series based). These data sets can become quite large, with high writes. Optimising for reads and writes are very different in Time-Series data when compared to traditional data workloads. High cardinality (column data with different values) is one of the bottlenecks for read performance.

One of abstraction created by TimescaleDB on top of Postgres is the **hypertable**. As a user, all inserts and reads are pushed to a single hypertable. Underneath, TimescaleDB handles partitioning across nodes to optimize write throughput and read performance.

# CitusDB

With the rise of Data Warehouses for OLAP use cases in 2000s, a lot of Data warehouse technologies were created. Many of these were forks of Postgres which changesto be compatible with OLAP workloads by making Postgres somehow column-oritend. Examples include ParAccel(AWS forked it into Redshift), Netezza(IBM acquired it), Vertica and Greenplum(VMware).

Citus is one of databases that is mixture of both OLTP and OLAP called **HTAP database**. It was acquired by Microsoft in 2019 and has been part of the Azure ecosystem.

So there you have it. Postgres is one of most exciting databases with various companies building platforms on top of Postgres!

# Postgres Extensions

Postgres has the ecosystem to be d tended for different usd cases. It is hookable to many layers of the stack, which makes it convenient for the Open source community to creates extensions that serve a particular niche purpose. Like with the rise of LLMs, there is a new breed of Vector databases that install Vector data types along with embeddings. Postgres has an extension for that called `pg-vector`.
...
Standard way of retireving/inserting data from other data sources, be it different Postgres server database, other databases such as Oracle/Mongo/SQL server
...
A neat tool I have found is [Tembo](https://tembo.io/) where you can install a lot of Postgres ecosystem with a Web dashboard.
